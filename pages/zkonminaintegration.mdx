...
> ⚠️ **Important note:** We don't actually support importing external dependencies other than the ones from `zkon-zkapp` and the `o1.js`. We're working on upgrading this.
...
> ⚠️ **Note:** For devnet there is no need to prepay for the requests.
